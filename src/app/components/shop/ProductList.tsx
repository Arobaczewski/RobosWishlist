...
export default function ProductListItem({
  ...
  const { addToCart: addToCartHook, closeCart } = useCart();
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    if (imageRef.current) {
      ...
      setFlyingImageSize({
        width: rect.width * 0.7,  // Start at 70% for list view
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg shadow-purple-500/20 dark:shadow-purple-500/30 hover:shadow-xl hover:shadow-purple-500/30 dark:hover:shadow-purple-500/40 transition-all duration-300 border border-gray-100 dark:border-gray-700 overflow-hidden h-full">
          ...
          <div className="flex flex-col lg:flex-row gap-6 p-6 h-full">
            ...
            <Link href={`/product/${product.id}`} className="lg:w-64 flex-shrink-0">
              <div ref={imageRef} className="relative aspect-square bg-gray-50 dark:bg-gray-700 rounded-lg overflow-hidden">
                <motion.div
                    ...
                    <div key={option.type}>
                      ...
                      {option.type === 'color' ? (
                        ...
                        <select
                          ...
                          className={`w-full max-w-xs px-3 py-2 text-sm border rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 ${
                            validationErrors[option.type]
